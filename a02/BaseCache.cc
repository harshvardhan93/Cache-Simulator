...
uint32_t BaseCache::getReads() { return numReads; }
...
bool BaseCache::read(uint32_t addr, uint32_t *data) {
  ...
  cout<<endl;
  cout<<"Tag: "<<tag<<"Index: "<<index<<"blockoffset: "<<blockoffset<<endl;
  for(unsigned int i=0;i<n_ways;i++) {
    if(cacheLines[index][i].valid==1) {
...
bool BaseCache::write(uint32_t addr, uint32_t data) {
  ...
  blockoffset=getdecblock(addr);
  for(unsigned int i=0;i<n_ways;i++) {
    if(cacheLines[index][i].tag==tag && cacheLines[index][i].valid==1) {
      //;
