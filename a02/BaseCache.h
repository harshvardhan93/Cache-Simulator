...
class BaseCache{
	...
	void lruupdate(unsigned int index,unsigned int i);
	uint32_t getbintag(uint32_t addr);
	uint32_t getdecindex(uint32_t addr);
	uint32_t getdecblock(uint32_t addr);
